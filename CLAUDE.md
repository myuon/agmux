...
- イシューの作業を行う場合、イシューの内容に曖昧なところがある場合にはユーザーと一緒にリファインを行うこと。リファインとは仕様を確定させたり方針の決定にはユーザーに質問を行うことでアラインメントを行う作業である。またリファインが完了するとは、エージェントだけで自律的に作業が完遂でき不確実なところがない状態を指す。リファインが終わったタスクは実装に着手すること
